...
fn main() {
    ...
    // Create the webview.
    let r = webview2::Environment::builder()
        // This is needed or `get_interface::<dyn ICoreWebView2Controller2>` will fail.
        // And you need dev/canary as of Feb 22, 2021.
        .with_target_compatible_browser_version("89.0.765")
        .build(move |env| {
        env.unwrap().create_controller(hwnd, move |c| {
            let c = c.unwrap();
            {
                use webview2_sys::ICoreWebView2Controller2;
                let c2 = c.as_inner().get_interface::<dyn ICoreWebView2Controller2>().unwrap();
                unsafe {
